...
# client.delete_collection("agric_guide")

db = Chroma(client=client, collection_name="agric_guide", embedding_function=OpenAIEmbeddings())
...
#                            persist_directory="./chroma_db", collection_name="agric_guide")
...
llm_response = conversational_rag_chain.invoke({"input": query},
                                               config={"configurable": {"session_id": "abc123"}})["answer"]

print("Reranked RAG ---------------------------", llm_response)

query = input("Enter a query again: ")

llm_response = conversational_rag_chain.invoke({"input": query},
                                               config={"configurable": {"session_id": "abc123"}})["answer"]

print("Reranked RAG 2 ---------------------------", llm_response)
